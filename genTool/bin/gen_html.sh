...
<input type="text" name="searching" value="搜索" />
</div>
</div>
...
function gen_topic_div_box {
   ...
   do
      ...
      list_other_lin="$(echo $_line | awk -F"\007" '{print $7}')"
...
######################################################################
echo "Generate the main pages and individual pages..."
...
join d_topic_type d
on   t.topic_type_id = d.topic_type_id
and  d.topic_type_desc = '$page_type'
join ( select topic_id, max(modified_ts) as modified_ts from f_topic_list group by 1 ) l
on t.topic_id = l.topic_id
order by 2 desc
;"	
	...
	cp $tmp_dir/all_topic_id.txt $tmp_dir/_topic_id.txt
	...
	do
		...
		if [ -n "$(echo $tmp_dir/topic_$topic_id.js)" ];then
